...
endpoint_urls = [["twitter", "facebook", "instagram", ],
...
retries_if_bad_data = 0  # value of one will contact endpoint twice in total
...
def social_network_activity():
    ...
    threads = []
    for i in range(num_endpoints):
        thread = threading.Thread(target=process_endpoint, args=[i])
...
def parse_json(json_text):
    ...
    try:
        received_json = json.loads(json_text)
    except ValueError:
        return False
...
def process_endpoint(endpoint_number):
    global return_json
    tries = 0
    successful = False
    while (tries <= retries_if_bad_data) and not successful:
        response = requests.get(endpoint_urls[1][endpoint_number], timeout=2)
        received_json = parse_json(response.text)
        # print("-------  TRY:", tries, "  --------  API:", api_number, "  -------\n", response.text, "\n^^^^^^^^^^^")
        if received_json:
            return_json[endpoint_urls[0][endpoint_number]] = len(received_json)
            successful = True
        else:
            tries = tries + 1
